...
FSMergeTree.prototype._mergeRelativePath = function(overwriteOptions, baseDir, possibleIndices) {
  ...
  for (i = 0; i < this.length; i++) {
    ...
    for (j = 0; j < names[i].length; j++) {
      ...
      const source = tree.findByRelativePath(filePath, { resolveSymlinks: false });
      const entry = Entry.clone(source.entry, filePath);

      entry.tree = tree;

      const isDirectory = findIfDirectory(entry);
      ...
      if (fileInfo[fileName] == null) {
        ...
        let existingEntryPath = existingEntry.tree.resolvePath(existingEntry.relativePath);
        ...
        if (originallyDirectory !== isDirectory) {
          ...
          throw new Error('Merge error: conflicting file types: ' + filePath
              ...
              + ' in ' + entry.tree.resolvePath(entry.relativePath) + '\n'
        ...
        if (!isDirectory && !overwrite) {
          throw new Error('Merge error: '
              ...
              + entry.tree.resolvePath(entry.relativePath) + ' and ' + existingEntryPath +'\n'
      ...
      if (infoHash.isDirectory) {
        if (infoHash.indices.length === 1 && canSymlink) {
          ...
          if (infoHash.indices[0] === i) { // avoid duplicate recursion
            ...
            // FSTree requires intermediate directory entries, so push
        ...
        // If the "directory" was a symlink before, it isn't now; it was either expanded or will be replaced with a new symlink.
        delete infoHash.entry._symlink;
      } else { // isFile
