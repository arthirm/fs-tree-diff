...
FSMergeTree.prototype._mergeRelativePath = function (overwriteOptions, baseDir, possibleIndices) {
  ...
  let names = this.map((tree, index) => {
    ...
    if (possibleIndices == null || possibleIndices.indexOf(index) !== -1) {
      // return tree.readdirSync(baseDir).sort();
       let a = tree.readdirSync(baseDir).sort();
      if(a.indexOf("app") > -1) {
        debugger;
        tree.readdirSync(baseDir).sort();
      }
      return a;
  ...
  for (i = 0; i < this.length; i++) {
    ...
    for (j = 0; j < names[i].length; j++) {
      ...
      let entry;
      if(filePath.indexOf("organization-global") > -1) {
        debugger;
      }
      let source = tree.findByRelativePath(filePath, { followSymlinks: false });
