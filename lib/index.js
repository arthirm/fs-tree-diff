  ...
  this.forEach(entry => {
    ...
    if (entry._symlink) {
      const linkedEntries = entry._symlink.tree.walkEntries().map(linkedEntry => {
        const relativePath = path.join(entry.relativePath, linkedEntry.relativePath);
      ...
      entries = entries.concat(this._filterItems(
        // treeToItems
        tree => linkedEntries, // substitute our own set of entries
        // itemToEntry
        entry => entry,
        // entryToItem
        entry => entry,
...
FSTree.prototype.changes = function(options) {
  ...
  return this._diffFilterChanges(options || {}).concat(this._processChanges(options || {}));
};

FSTree.prototype._diffFilterChanges = function(/* options */) {
  ...
  this._changes.forEach(change => {
    ...
    if (change[2]._symlink) {
      ...
      combinedChanges = combinedChanges.concat(this._filterItems(
        // treeToItems
        tree => linkedChanges, // substitute our own set of entries
        // itemToEntry
        change => change[2],
        // entryToItem
