...
FSTree.prototype = {
  get _changes() {
    const rawChanges = this.parent ? this.parent._changes : this.__changes;
    const filteredChanges = this._filterItems(rawChanges, change => change[2]);
  ...
  get _entries() {
    const rawEntries = this.parent ? this.parent._entries : this._rawEntries;
    const filteredEntries = this._filterItems(rawEntries, entry => entry);
  ...
  get root() {
    ...
    return ensureTrailingSlash(this._root);
  },
...
FSTree.prototype._filterItems = function(rawItems, itemToEntry) {
  let dirStack = [];
  let dirDepth = 0;
  const filteredItems = [];
  ...
  rawItems.forEach(item => {
    ...
    if (filterMatches(entry.relativePath, this._cwd, this.files, this.include, this.exclude)) {
