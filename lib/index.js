  ...
  inputsArr.forEach(input => {
    ...
    if (filterMatched) {
      ...
      for (let i = 0; i < dirStack.length; i++) {
        if(entry.relativePath.indexOf(dirStack[i].relativePath) === 0 ) {
...
FSTree.prototype.changes = function(options) {
  ...
  if (shouldDiff) {
    return diffToFindPatches(this, options);
  } else {
    return processChanges(this, options);
  }
};

function diffToFindPatches(tree, options) {
  ...
  const filteredEntries = tree._filterEntries(options);
...
function processChanges(tree, options) {
  let filteredEntries = [];
  let dirStack = [];
  let dirDepth = 0;
  ...
  tree._changes.forEach(change => {
    ...
    if (change[2]._projection) {
       matchingEntries = processChangeWithProjections(tree, change, options);
      if (matchingEntries.length > 0) {
        matchingEntries = postProcessPatchesFromProjections(change, matchingEntries, tree.cwd)
      }
    } else {
      matchingEntries = processChangeWithoutProjection(tree, change, options);
    }
    ...
    if (matchingEntries.length > 0) {
      ...
      if (topElem !== undefined) {
        let lastEntry = topElem[2];
        let lastDepth = getDirDepth(lastEntry.relativePath);
        const nextEntry = matchingEntries[0][2];
        const nextDepth = getDirDepth(nextEntry.relativePath);

        if (isDirectory(nextEntry)) {
          while (filteredEntries.length !== 0 && isDirectory(lastEntry) && lastDepth >= nextDepth) {
            filteredEntries.pop();
            lastEntry = filteredEntries[filteredEntries.length - 1][2];
            lastDepth = getDirDepth(lastEntry.relativePath);
          }
        }
      }

      filteredEntries = filteredEntries.concat(matchingEntries);
    } else if (isDirectory(change[2])) {
      const curDirDepth = getDirDepth(entry.relativePath);
    ...
    if (filteredEntries.length > 0) {
      ...
      filteredEntries.forEach(entry => {
        if (entry[1].startsWith(tree.cwd)) {
          entry[1] = entry[1].replace(tree.cwd, '');
          let newEntry = Entry.cloneEntry(entry[2]);
          newEntry.relativePath = newEntry.relativePath.replace(tree.cwd, '');
          entry[2] = newEntry;
        }
      });
...
function processChangeWithProjections(tree, change, options) {
...
function processChangeWithoutProjection(tree, change, options) {
...
function postProcessPatchesFromProjections(change, projectedEntries, cwd) {
