...
FSTree.prototype = {
  ...
  get _unfilteredEntries() {  // TODO: rename?
    ...
    return entries.filter(entry => {
      return entry.relativePath !== this.cwd && entry.relativePath.startsWith(this.cwd);
...
FSTree.prototype._filterItems = function(treeToItems, itemToEntry, replaceRelativePath) {
  ...
  for (let i = 0; i < rawItems.length; i++) {
    ...
    if (isSymlink(entry) && isDirectory(entry) && this.cwd.startsWith(ensurePathSep(entry.relativePath))) {
      ...
      const changes = rootedTree._filterItems(treeToItems, itemToEntry, replaceRelativePath).map(item => {
        return replaceRelativePath(item, path.join(this.cwd, itemToEntry(item).relativePath));
      }).filter(item => {
        return this._filterMatches(itemToEntry(item).relativePath);
      }).map(item => replaceRelativePath(item, itemToEntry(item).relativePath.replace(this.cwd, '')));
...
FSTree.prototype.findByRelativePath = function(relativePath, options) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    } else if (symlink && normalizedPath.startsWith(ensurePathSep(entry.relativePath))) {
...
FSTree.prototype.readdirSync = function(relativePath) {
  ...
  const names = tree._entries.filter(entry => chompPathSep(entry.relativePath).indexOf(path.sep) === -1).map(entry => chompPathSep(entry.relativePath));
...
FSTree.prototype.symlinkSyncFromEntry = function(srcFSTree, srcRelativePath, destRelativePath) {
  ...
  if (destNormalizedPath === '') {
    ...
    if (this._unfilteredEntries.length) {
...
FSTree.prototype.emptySync = function(relativePath) {
  ...
  if (result.entry === ROOT) {
    unfilteredEntries = this._unfilteredEntries;
  } else {
    tempTree = this.chdir(result.entry.relativePath);
    unfilteredEntries = tempTree._unfilteredEntries.map(entry => {
...
FSTree.prototype._ensureEntriesPopulated = function() {
  ...
  // Must use an empty array for rootless trees to preserve old API behavior
  // `tree.calculatePatch(new FSTree())` (e.g. ember-browserify stub generator).
  ...
  if (this.files && !this.include.length && !this.exclude.length) {
    this._rawEntries = this._processedFiles.map(e => Entry.fromPath(e));
    this._rawEntries = this._rawEntries.map(entry => {
      entry.relativePath   = path.join(this.cwd, entry.relativePath)
    });
  ...
  // if (this.include.length && this.include.every(include => typeof include === 'string')) {
  //   walkSyncOptions.globs = this.include;
  // }

  // if (this.exclude.length && this.exclude.every(exclude => typeof exclude === 'string')) {
  //   walkSyncOptions.ignore = this.exclude;
  // }

  this._rawEntries = walkSync.entries(this.root, walkSyncOptions);

  // TODO: prefix cwd to all entries
  this._rawEntries = this._rawEntries.map(entry => {
    entry.relativePath   = path.join(this.cwd, entry.relativePath)
  });
...
function match(path, matcher) {
  ...
  throw new Error(`Matcher must be a Minimatch, RegExp, string, or function.  Got '${typeof matcher}'.`);
}
