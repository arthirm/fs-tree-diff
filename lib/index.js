...
FSTree.prototype = {
  ...
  set files(value) {
    ...
    if (newValue && this._files && newValue.length === this._files.length && newValue.every((v, i) => v === this._files[i])) {
  ...
  get root() {
    ...
    return this._root ? ensurePathSep(this._root) : '';
  },
...
FSTree.prototype._setFilter = function(filter, value) {
  ...
  if (newValue && this[`_${filter}`] && newValue.length === this[`_${filter}`].length && newValue.every((v, i) => v === this[`_${filter}`][i])) {
    // No change; no need to reset caches, etc.
    return;
  }
...
FSTree.prototype._filterItems = function(treeToItems, itemToEntry, entryToItem, replaceRelativePath) {
  ...
  for (let i = 0; i < rawItems.length; i++) {
    ...
    if (entry._symlink && this.cwd.startsWith(ensurePathSep(entry.relativePath))) {
    ...
    if (this._filterMatches(entry.relativePath)) {
      ...
      if (filteredItems.length) {
        const relativePath = isDirectory(entry) ? ensurePathSep(entry.relativePath) : entry.relativePath;
        let lastItem = filteredItems[filteredItems.length - 1];
        let lastEntry = itemToEntry(lastItem);

        while (filteredItems.length && !explicitlyMatchedItems.has(lastItem) && isDirectory(lastEntry) && !relativePath.startsWith(ensurePathSep(lastEntry.relativePath))) {
          filteredItems.pop();
...
FSTree.prototype.emptySync = function(relativePath) {
  ...
  const unfilteredEntries = this._unfilteredEntries.reverse();
...
function shouldFollowSymlink(entryPath, cwd) {
  ...
  var val = cwd === entryPath || entryPath.startsWith(cwd) || cwd.startsWith(ensurePathSep(entryPath));
  return val;
}
...
function filterMatches(relativePath, excludedDirectoriesCache, cwd, files, include, exclude) {
  ...
  if (files) {
    //TODO: Handle the case where files start with ./ 
