...
FSTree.prototype = {
  get _changes() {
    const filteredChanges = this._filterItems(tree => {
      return tree.parent ? tree.parent._changes : tree.__changes;
    }, change => change[2], (change, relativePath) => [change[0], relativePath, Entry.clone(change[2], relativePath)]);
  ...
  get _entries() {
    const filteredEntries = this._filterItems(tree => {
      return tree.parent ? tree.parent._entries : tree._rawEntries
    }, entry => entry, (entry, relativePath) => Entry.clone(entry, relativePath));
...
FSTree.prototype._filterItems = function(treeToItems, itemToEntry, replaceRelativePath) {
  ...
  treeToItems(this).forEach(item => {
    ...
    if (entry._symlink && this.cwd.startsWith(ensureTrailingSlash(entry.relativePath))) {
      ...
      const linkedItems = rootedTree._filterItems(treeToItems, itemToEntry);

      filteredItems = filteredItems.concat(linkedItems.map(item => {
        let newEntry = itemToEntry(item);
        const newRelativePath = path.join(entry.relativePath, newEntry.relativePath);

        return replaceRelativePath(item, newRelativePath);
      }));
    }
    ...
    if (filterMatches(entry.relativePath, this.cwd, this.files, this.include, this.exclude)) {
      ...
      let lastEntry = itemToEntry(filteredItems[filteredItems.length - 1]);

      while (filteredItems.length && !explicitlyMatchedItems.has(lastEntry) && isDirectory(lastEntry) && !relativePath.startsWith(ensureTrailingSlash(lastEntry.relativePath))) {
        filteredItems.pop();
        lastEntry = itemToEntry(filteredItems[filteredItems.length - 1]);
      }

      // Add non-matched directories which contain matching files.
      dirStack.forEach(dir => {
        if (entry.relativePath.startsWith(ensureTrailingSlash(dir.relativePath))) {
...
FSTree._processChanges = function() {
  const traversedSymlinks = new Set();
  let combinedChanges = [];

  this._changes.forEach(change => {change[2]._symlink.tree.walkEntries()});

  this._entries.forEach(entry => {entry._symlink.tree.changes()});

  return combinedChanges;
}

function diffToFindPatches(tree, filteredEntries, options) {
  let filtersForFiles = tree.files.length !== 0 && tree.include.length === 0 && tree.exclude.length === 0;
  if (filtersForFiles) {
  ...
  if (tree.cwd) {
    return patches.map(patch => {
      ...
      let newEntry = Entry.clone(patch[2]);
...
FSTree.prototype._throwIfNotWritable = function(operation, relativePath) {
  ...
  if (typeof relativePath !== 'undefined') {
    ...
    if (operation !== 'symlinkSyncFromEntry' || normalizedPath !== '') {
      ...
      if (operation !== 'mkdirp') {
        const parentResult = this.findByRelativePath(path.dirname(normalizedPath));
